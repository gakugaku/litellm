...
pub(super) async fn signed_headers(
    ...
    use crate::providers::bedrock::aws_base::{
        aws_auth_config, aws_signature_headers, host_supplied_credentials, resolve_credentials,
        sign_bedrock_post,
    };
    ...
    // Every original header goes back on the wire, with the computed SigV4 ones
    // winning on collision, which is the order Python reattaches them in.
