...
def newrelic_dynamic_endpoint(params: StandardCallbackDynamicParams) -> str | None:
    """Per-request OTLP endpoint for the team's ``newrelic_region``, or ``None``.

    ``None`` keeps the preset's default (US) endpoint. An unknown region also
    falls back to the default rather than exporting to a guessed host.
    """
