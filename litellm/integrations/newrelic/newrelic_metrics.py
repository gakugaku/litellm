...
class NewRelicMetricsLogger(CustomBatchLogger):
    ...
    async def async_send_batch(self) -> None:
        ...
        self.log_queue = []  # mutable-ok: CustomBatchLogger's log_queue contract requires a list it can mutate
    ...
    def _requeue(self, batch: tuple[NewRelicMetricRecord, ...]) -> None:
        requeued: Final = (*batch, *self.log_queue)
        overflow: Final = len(requeued) - self.max_queue_size
        kept: Final = requeued[overflow:] if overflow > 0 else requeued
        self.log_queue = list(kept)  # mutable-ok: CustomBatchLogger's log_queue contract requires a list it can mutate
        if overflow > 0:
            verbose_logger.warning(
